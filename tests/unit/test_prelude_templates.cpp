...
TEST(PreludeTemplates, ArithmeticOps) {
  int a = 6, b = 3;
  // Arithmetic - use parentheses to protect template commas from macro
...
TEST(PreludeTemplates, AssignmentAndComparisons) {
  ...
  EXPECT_EQ(x, 4);
  EXPECT_EQ((&optiweave::__primop_mod_assign<int, int>()(x, 3)), &x);
  EXPECT_EQ(x, 1);
  // Comparisons - after mod_assign, x=1, y=2
  EXPECT_FALSE((optiweave::__primop_eq<int, int>()(x, y)));  // 1 != 2
  EXPECT_TRUE((optiweave::__primop_ne<int, int>()(x, y)));   // 1 != 2
  EXPECT_TRUE((optiweave::__primop_lt<int, int>()(x, y)));   // 1 < 2
  EXPECT_FALSE((optiweave::__primop_gt<int, int>()(x, y)));  // 1 > 2
  EXPECT_TRUE((optiweave::__primop_le<int, int>()(x, y)));   // 1 <= 2
  EXPECT_FALSE((optiweave::__primop_ge<int, int>()(x, y)));  // 1 >= 2
}
