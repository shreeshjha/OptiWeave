...
void apply_blur(int* image, int width, int height) {
    ...
    for (int y = 1; y < optiweave::ow_sub(height, 1); y++) {
        for (int x = 1; x < optiweave::ow_sub(width, 1); x++) {
            ...
            (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 50, __FUNCTION__), temp[y * width + x] = sum / 9);
        }
    }
...
void apply_threshold(int* image, int width, int height, int threshold) {
    ...
    for (int i = 0; i < optiweave::ow_mul(width, height); i++) {
        (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 61, __FUNCTION__), image[i] = (image[i] > threshold) ? 255 : 0);
    }
}
...
void apply_edge_detect(int* image, int width, int height) {
    ...
    for (int y = 1; y < optiweave::ow_sub(height, 1); y++) {
        for (int x = 1; x < optiweave::ow_sub(width, 1); x++) {
            ...
            (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 80, __FUNCTION__), temp[y * width + x] = std::min(255, (int)std::sqrt(gx*gx + gy*gy)));
        }
    }
...
void run_demo() {
    ...
    // Initialize
    for (int i = 0; i < optiweave::ow_mul(WIDTH, HEIGHT); i++) {
        (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 98, __FUNCTION__), image[i] = rand() % 256);
    }
...
void run_demo() {
    ...
    for (int i = 0; i < SIZE; i++) {
        (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 165, __FUNCTION__), matrix[i] = new int[SIZE]);
        for (int j = 0; j < SIZE; j++) {
            (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 167, __FUNCTION__), matrix[i][j] = i + j);
        }
    }
...
void run_demo() {
    ...
    const int N = 10000;
    int* data = new int[N];
    for (int i = 0; i < N; i++) (__optiweave_record_subscript("/root/testing/OptiWeave/evaluation/instrumentation_use_cases/use_case_demo.cpp", 349, __FUNCTION__), data[i] = i);
