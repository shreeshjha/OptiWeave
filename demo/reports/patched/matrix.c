...
void normalize(float *x, int n, float scale) {
        const double __ow_recip_1 = 1.0 / (scale); /* OPTIWEAVE: ensure divisor != 0 */
for (int i = 0; i < n; i++) {
        x[i] = x[i] * __ow_recip_1;               /* <-- division in loop body */
    }
}
...
void gather_strided(const float *src, float *dst, int n, int stride) {
        // OPTIWEAVE: Strength reduction opportunity
    // Replace i*K multiplication with accumulator += K
for (int i = 0; i < n; i++) {
...
void transpose(float dst[M][M], const float src[M][M]) {
    for (int i = 0; i < M; i++) {       /* outer walks columns */
        for (int j = 0; j < M; j++) {   /* inner walks rows of src */
